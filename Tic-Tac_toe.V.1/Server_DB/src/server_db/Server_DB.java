...
public class Server_DB {
    ...
    Server_DB() {
        try {
            ...
            Logger.getLogger(Server_DB.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
        ...
        if (msg != null) {
            ...
            switch (incommingMsg[0]) {
                ...
                case "l": // login verfication  
                    ...
                    gamerHandler.userName = incommingMsg[1];
                    if (TestUser.checkUsername(gamerHandler.userName)) 
                    {
                        ps.println("l:a");
                    } else {
                        ps.println("l:n");
                ...
                case "lp":
                  ...
                  if (!(gamerHandler.userName.equals(""))) {
                        if (TestUser.checkUsername(gamerHandler.userName, pass)) {
                            ps.println("lp:a");
                                // sendMessageToAll();
                        } else {
                            ps.println("lp:n");
            ...
            gamerHandler.userName = null; // to prevent this msg from printing 
            sendMessageToAll();
            stop();
