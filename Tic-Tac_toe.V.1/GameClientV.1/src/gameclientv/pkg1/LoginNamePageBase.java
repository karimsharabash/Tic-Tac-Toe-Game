...
public class LoginNamePageBase extends AnchorPane {
    ...
    static String oldLoginUserName;
    static String oldLoginUserPass;

    static boolean userLoginFlag = false;
    static boolean passLoginFlag = false;

    public LoginNamePageBase(GameClientV1 client, Stage primaryStage) {
        ...
        imageView3.setImage(new Image(getClass().getResource("img/login.gif").toExternalForm()));
        ...
        NameTxtField.focusedProperty().addListener(new ChangeListener<Boolean>() {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                if (!newValue) {
                    ...
                    if (!TextfieldContent.equals("") && !TextfieldContent.equals(oldLoginUserName)) {
                        oldLoginUserName = TextfieldContent;
                        client.ps.println("l:" + TextfieldContent);
                    }
        ...
        PassTxtField.focusedProperty().addListener(new ChangeListener<Boolean>() {
            ...
            public void changed(ObservableValue<? extends Boolean> observable, Boolean oldValue, Boolean newValue) {
                if (!newValue) {
                    ...
                    if (!TextfieldContent.equals("") && !TextfieldContent.equals(oldLoginUserPass)) {
                        oldLoginUserName = TextfieldContent;
                        client.ps.println("lp:" + TextfieldContent);
                    }
                }
            }
        ...
        SubmitPane.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {
                 SubmitPane.requestFocus();
                try {
                    Thread.sleep(100);
                } catch (InterruptedException ex) {
                    Logger.getLogger(LoginNamePageBase.class.getName()).log(Level.SEVERE, null, ex);
                }
                System.out.println("fire event submit");
                if (userLoginFlag == true) {
                    System.out.println("user flag" + userLoginFlag);
                    if (passLoginFlag == true) {
                        System.out.println("pass flag" + passLoginFlag);
                        Select_Login_SignupBase.Login(client, primaryStage);
                    } else {
                        Select_Login_SignupBase.alertMsg("enter right password ");
                    }
                } else {
