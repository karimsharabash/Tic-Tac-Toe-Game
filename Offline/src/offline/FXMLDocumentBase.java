...
public class FXMLDocumentBase extends AnchorPane {
    ...
    static void game(Pane region, int place) {
        ...
        if (OfflineGame.xTurn) {
            img = OfflineGame.playerOneTurn(place);
            xWin = OfflineGame.checkWin(OfflineGame.xPlaces);
            System.out.println("x " + xWin);

        } else if (!OfflineGame.offlineGameMode) {
            img = OfflineGame.playerTwoTurn((place));
            oWin = OfflineGame.checkWin(OfflineGame.oPlaces);
        }
        if (img != null) {
            region.getChildren().add(img);
            OfflineGame.xTurn = !OfflineGame.xTurn;
            if (xWin) {
                gameResult("X Won");
                return;
            } else if (oWin) {
            ...
            if (OfflineGame.offlineGameMode) {
                newPlace = OfflineGame.computerPlay();
                img = OfflineGame.playerTwoTurn(newPlace);
                oWin = OfflineGame.checkWin(OfflineGame.oPlaces);
    ...
    private static void gameResult(String msg) {
